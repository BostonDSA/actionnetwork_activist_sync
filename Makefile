...
local-ingest-run:
	uv run python-lambda-local -f lambda_handler lambda_ingester.py samples/lambda_ingester_event.json

# Developer Processor

local-processor:
	uv run python-lambda-local -f lambda_handler lambda_processor.py samples/lambda_processor_event.json

# Developer Lapsed Cron

local-lapsed:
	uv run python-lambda-local -f lambda_handler lambda_lapsed.py samples/lambda_lapsed_event.json

# Developer Neighborhood Cron

local-hood:
	uv run python-lambda-local -t 60 -f lambda_handler lambda_neighborhoods.py samples/lambda_neighborhoods_event.json
